...
class MysqlZfsService(object):
    ...
    def __init__(self, opts, snapshot=None):
    ...
    def write_cnf(self):
        ...
        if os.path.isfile(cnf_file_restored) and os.path.isfile(self.cnf_file) and \
            ...
            logger.debug('Looks like my.cnf restored from ZFS snapshot, backing up')
    ...
    def stop(self):
        if not self.is_alive():
            logger.info('MySQL does not seem to be running, not doing anything')
            return True

        logger.info('Stopping MySQL service on %s' % self.rootdir)

        mysqld = ['/usr/bin/mysqladmin', '--defaults-file=%s' % self.opts.dotmycnf,
                  '--socket', self.cnf['mysqld']['socket'], 'shutdown']
        logger.debug('Shutting down mysqld with %s' % ' '.join(mysqld))
        ...
        while r is None:
            ...
            if pid_exists:
                ...
                if poll_count > 5:
                    ...
                    if log_size != poll_size:
                        logger.info('.. we are not stuck, mysqld is shutting down')
                        poll_count = 0
                        poll_size = log_size
                        continue
                    else:
                        logger.info('Giving up waiting on PID, we\'ll keep checking connection')
                        pid_exists = False
                        continue

                if not pid_exists:
                    logger.info('MySQL PID removed, almost there ...')
                    poll_count = 0
            ...
            if poll_count > 30 or not can_connect:
                logger.debug('Exceeded poll count %d' % poll_count)
                if not can_connect:
                    r = p_mysqld.wait()
                ...
                can_connect = self.is_alive()
                if can_connect:
                    logger.info('MySQL still running, giving up, please shutdown manually')
                    r = 9999
                    break

        if can_connect:
            logger.info('MySQL is up, connect with command:')
            logger.info('mysql --defaults-file=%s --socket %s' % (
                            self.opts.dotmycnf, self.cnf['mysqld']['socket']))

        if r != 0:
            logger.error('Stopping mysqld returned with bad code: %d' % r)
            logger.error('Check error log at %s' % self.cnf['mysqld']['log-error'])
            return False
    ...
    def start(self):
        ...
        self.write_cnf()
        if self.is_alive():
            logger.info('MySQL is already running on this dataset, connect with:')
            logger.info('mysql --defaults-file=%s --socket %s' % (
                             self.opts.dotmycnf, self.cnf['mysqld'][socket]))

        logger.info('Starting MySQL service on %s' % self.rootdir)
        ...
        logger.debug('Starting mysqld with %s' % ' '.join(mysqld))
        logger.info('You can tail the error log at %s' % self.cnf['mysqld']['log-error'])
        ...
        if pid_no is None:
            ...
            if pid_no is not None:
                logger.info('Using PID from from pstree %d' % int(pid_no))

        if pid_no is None:
            pid_no = self.pid_from_log(self.cnf['mysqld']['log-error'])
            logger.info('Using PID from from error log %d' % int(pid_no))

        if not self.wait_for_io(pid_no):
            logger.error('MySQL process has had no IO activity')
        ...
        while r is None:
            ...
            if poll_count > 30:
                logger.info('Still cannot connect, giving up ...')
                r = 9999
                break
        ...
        if r != 0:
            logger.error('Starting mysqld returned with bad code: %d' % r)
            logger.error('Check error log at %s' % self.cnf['mysqld']['log-error'])
            return False
    ...
    def is_alive(self):
        ...
        try:
            ...
            try:
                ...
                logger.error('MySQL is running but could not execute query')
                return None
    ...
    def wait_for_io(self, pidno, polls = 5):
        ...
        while True:
            ...
            logger.debug('Old read bytes: %d, new read bytes: %d' % (
            ...
            poll = poll + 1
            logger.debug('wait_for_io polls %d' % poll)
            if poll > polls:
                return False
    ...
    def wait_for_pid(self, pidfile, errorlog, polls=2):
        ...
        while True:
            ...
            logger.debug('Old log size: %d, new log size: %d' % (size_prev, size_curr))
            ...
            poll = poll + 1
            logger.debug('wait_for_pid polls %d' % poll)
            size_prev = size_curr
            if poll > polls:
                logger.info('Timed out waiting for PID file to be created')
                return None
